 ...
 *
 * Proves the Lap→Engraving cycle asset exists with the contract shape
 * (68 frames, ~2.266667s, 2160x1080, 30 fps, H.264/yuv420p, no audio).
 * Does not claim visual edit order — that is an independent rendered check.
 *
...
const assetRel = "assets/studio-hand-work-cycle.mp4";
...
if (probe.status !== 0) {
  ...
  fail("ffprobe failed on hand work-cycle asset");
}
...
try {
  ...
  fail("ffprobe JSON parse failed");
}
...
if (video.length !== 1) {
  fail(`expected exactly one video stream, found ${video.length}`);
}
if (audio.length !== 0) {
  fail(`expected no audio streams, found ${audio.length}`);
}
...
if (v.codec_name !== "h264") fail(`codec_name must be h264, got ${v.codec_name}`);
if (v.pix_fmt !== "yuv420p") fail(`pix_fmt must be yuv420p, got ${v.pix_fmt}`);
if (Number(v.width) !== 2160) fail(`width must be 2160, got ${v.width}`);
if (Number(v.height) !== 1080) fail(`height must be 1080, got ${v.height}`);
if (rate !== "30/1") fail(`r_frame_rate must be 30/1, got ${rate}`);
if (!Number.isFinite(frames) || frames !== 68) {
  fail(`frame count must be exactly 68, got ${framesRaw}`);
}
if (!Number.isFinite(duration)) fail(`duration must be numeric, got ${info.format && info.format.duration}`);
// 68/30 = 2.2666…; allow tiny container rounding only.
if (Math.abs(duration - 68 / 30) > 0.0005) {
  fail(`duration must be ~2.266667s (68/30), got ${duration}`);
}

console.log(
  "PASS: hand work-cycle media (68 frames; ~2.266667s; 2160x1080; 30fps; h264/yuv420p; no audio)"
);
